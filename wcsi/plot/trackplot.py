...
def track_overview(track=None, ib=None):
    ...
    if ib is not None:
        ...
        idx = 0
        for nat_, npoints in [
            (a[0], sum([1 for _ in a[1]])) for a in groupby(ib.nature.values)
        ]:
            if nat_ in natures_ib:
                color = natures_ib[nat_][1]
            else:
                color = natures_ib["nTC"][1]

            ib_ = ib.isel(record=slice(idx, min(idx + npoints + 1, len(ib.time))))

            if len(ib_.record) > 1:
                ib_.hrcn.plot_fancyline(colors="w", ax=axes["A"], linewidths=3)
                ib_.hrcn.plot_fancyline(colors=color, ax=axes["A"], linewidths=2)

            idx += npoints

    for nlabel in natures:
        axes["l"].fill_betweenx(
            [np.nan, np.nan],
            np.nan,
            np.nan,
            color=natures[nlabel][1],
            label=natures[nlabel][0],
        )
    axes["l"].legend(title="ERA5 Nature", bbox_to_anchor=[0.75, 0.8])

    for nlabel in natures_ib:
        axes["m"].fill_betweenx(
            [np.nan, np.nan],
            np.nan,
            np.nan,
            color=natures_ib[nlabel][1],
            label=natures_ib[nlabel][0],
        )
    if ib is not None:
        axes["m"].legend(title="IBTrACS Nature", bbox_to_anchor=[0.75, 0.6])
    ...
    for ax_label in ["B", "C", "D", "E", "F", "G"]:
        ...
        if track is not None:
            ...
            for nat_, npoints in [
                ...
                color = natures[nat_][1]

                track_ = track.isel(record=slice(idx, min(idx + npoints, len(track.time))))
                times = pd.to_datetime(track_.time)
                ax.fill_betweenx(
                    [y[0], y[1]],
                    times.min() - timedelta(hours=3),
                    times.max() + timedelta(hours=3),
                    color=color,
                    ec=None,
                    alpha=0.4,
                )
    ...
    for ax_label in ["x", "y"]:
        ...
        if ib is not None:
            ...
            for nat_, npoints in [
                ...
                ib_ = ib.isel(record=slice(idx, min(idx + npoints, len(ib.time))))
                times = pd.to_datetime(ib_.time)
                ax.fill_betweenx(
                    [y[0], y[1]],
                    times.min() - timedelta(hours=3),
                    times.max() + timedelta(hours=3),
...
def _main(tracks, ibtracs, superbt, table):
    ...
    plt.savefig(f"invest_b8l_2013.pdf")
    plt.close()
